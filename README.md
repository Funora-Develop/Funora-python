...
Работают тридцать две операции: двадцать одна чтения и одиннадцать записи - отправка текста и картинки, отметка прочтения, отзыв и его снятие, правка цены лота, поднятие предложений, включение и выключение лота.
...
Отправка изображения, отметка прочитанным, дочитывание переписки, схема полей
раздела, история операций счёта, лента событий заказа, язык интерфейса и две
операции записи над лотами - включение и выключение - объявлены контрактом и не
написаны: запросов, которыми площадка их делает, никто не наблюдал. Позвав их,
вы получите `NotImplementedOperationError` - отказ самой Funora, а не встроенную
ошибку Python.

Полный перечень с причиной у каждой строки лежит в реестре
`spec/conformance/not-implemented.yaml`. Это не справочник, а условие сборки:
объявленная и молча отсутствующая операция ворота не проходит.

## Что наблюдено, но операцией не стало

Три точки записи площадки наблюдены **формами** - адрес и поля видны, а сам
запрос никто не отправлял:

| Точка | Что это | Чего не хватает |
|---|---|---|
| `POST /orders/refund` | возврат по заказу | ответа на отправку |
| `POST /withdraw/withdraw` | вывод средств | ответа; нужен второй множитель |
| `POST /users/transactions` | догрузка операций счёта | смысла курсора `continue` |

Операции записи, не умеющей отличить успех от отказа, здесь не будет: она
сообщает об успехе всегда. Возврат и вывод к тому же необратимы и оба про
деньги.

Записано в `spec/extraction`, наблюдения перечислены в
[docs/observation-plan.md](docs/observation-plan.md).

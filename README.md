...
Рабочую версию можно установить из исходников или собрать wheel.
Контракт пока имеет статус draft; сборка пакета не означает проверку всех
операций на действующем аккаунте.

Реализованы и проверяются тестами тридцать четыре операции: двадцать три чтения и одиннадцать записи - отправка текста и картинки, отметка прочтения, отзыв и его снятие, правка цены лота, поднятие предложений, включение и выключение лота, смена валюты интерфейса и возврат по заказу.
...
- Вывод средств: форма наблюдена, но результат отправки и второй фактор не разобраны.
...
- Полная изоляция публичного чтения по отдельному транспорту и отмена уже
  отправленного запроса ради более приоритетного.

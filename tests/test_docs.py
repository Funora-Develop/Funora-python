...
def test_every_result_type_is_importable_from_the_package() -> None:
    ...
    # None вывозить неоткуда и незачем: это встроенное имя языка, а не наш тип.
    # Появилось оно с первой операцией, объявленной в контракте как void, -
    # отметкой прочтения, у которой площадка подтверждения не даёт вовсе.
    published.add("None")
...
def test_the_readme_counts_the_operations_it_has() -> None:
    ...
    words = {
        ...
        22: ("двадцать две", "twenty-two"),
        23: ("двадцать три", "twenty-three"),
        24: ("двадцать четыре", "twenty-four"),
        25: ("двадцать пять", "twenty-five"),
        26: ("двадцать шесть", "twenty-six"),
        27: ("двадцать семь", "twenty-seven"),
        28: ("двадцать восемь", "twenty-eight"),
        29: ("двадцать девять", "twenty-nine"),
        30: ("тридцать", "thirty"),
        31: ("тридцать одну", "thirty-one"),
        32: ("тридцать две", "thirty-two"),
        33: ("тридцать три", "thirty-three"),
        34: ("тридцать четыре", "thirty-four"),
        35: ("тридцать пять", "thirty-five"),
    }

...
def test_every_generated_selector_is_read() -> None:
    ...
    known_silent = {
        ...
        # То же, что у рынка лотов, и по той же причине: селектор при признаке
        # стоит затем, чтобы ворота искали АТРИБУТ на строке, а не по всему
        # документу. Сама строка читается один раз - через chips.rows.
        "chips.markers.online",
        # Признак выключенного лота: селектор при нём стоит затем, чтобы ворота
        # искали КЛАСС на строке, а не по всему документу. Сама строка читается
        # один раз - через lots.rows, тем же селектором.
        #
        # Класс при этом ЧИТАЕТСЯ - литералом в _own_lots.py, как и offer-promo
        # в разборе рынка: генератор классы-признаки не собирает вовсе. Собрать
        # их в таблицу стоит, и это отдельная работа.
        "lots.visibility.marker",
        # Схема полей раздела: наблюдена 31.08.2026, операции catalog.field_schema
        # под ней ещё нет. Селекторы объявлены ради самого наблюдения - чтобы
        # ворота сверяли их со снимком и заметили, если разметка уедет.
        #
        # Операция появится, когда решится, ЧТО она возвращает: видов поля
        # наблюдено два - выбор и диапазон, - а бывают ли другие, неизвестно, и
        # объявлять перечень закрытым по одному разделу нельзя.
        "catalog.field_schema.container",
        "catalog.field_schema.fields",
        "catalog.field_schema.input",
        "catalog.field_schema.kinds.choice",
        "catalog.field_schema.kinds.range",
        "catalog.field_schema.label",

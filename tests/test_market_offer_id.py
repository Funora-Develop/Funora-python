...
ЗАЧЕМ ОТДЕЛЬНЫЙ НАБОР. Полтора года - фигура речи, но история короткая и
поучительная. В реестре неисполненного про market.offers стояло: «идентификатор
чужого предложения лежит в строке запроса ссылки и атрибутом не выносится», и
вывод - «в проекте его нет ниоткуда».
...
def test_every_row_carries_an_offer_id_parameter() -> None:
    ...
    without = [
        one for one in rows if "id=" not in ((one.attributes or {}).get("href") or "")
    ]

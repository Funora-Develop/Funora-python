...
def test_the_header_reaches_the_identity(no_sleep: list[float]) -> None:
    ...
    # Допуск в миллисекунду не послабление, а следствие двоичной дроби:
    # 1120.742512378 - 820.742512378 даёт 299.99999999999994, и проверка
    # показывала отказ там, где реализация права. Различить пять минут и одну
    # миллисекунда не мешает - она на пять порядков меньше того, что здесь
    # различается.
    assert identity.cooldown_until - started >= 300.0 - 1e-3, (
...
def test_absent_service_refuses_by_contract_not_by_language() -> None:
    """Проверяет, что видимость службы совпадает с её написанностью.

    Служб в контракте шесть, и на 31.08.2026 написаны ВСЕ ШЕСТЬ: рынок был
    последним. Прежде обращение к ненаписанной давало голый AttributeError -
    бот, обернувший работу в except FunoraError, падал не операцией, а всем
    процессом, и узнавал причину из трассировки.
    ...
    Опечатка при этом обязана остаться языковой ошибкой: выдавать её за
    объявленную операцию значило бы отправить человека искать в реестре то,
    чего он не писал.
    ...
    with _client([]) as client:
        ...
        assert hasattr(client, "market"), (
            "служба рынка написана с 31.08.2026: публичный список раздела "
            "читается. Снятие отсюда - не косметика: пока она числилась "
            "ненаписанной, обращение к ней отказывало по контракту"
        )

        # ОТКАЗ ПРОВЕРЯЕТСЯ НА СЛУЖБЕ, КОТОРОЙ ВПРАВДУ НЕТ. Прежде здесь стоял
        # рынок; он написан, и проверка вместе с ним проверяла бы пустоту.
        #
        # Служба сообщений - chats - написана, служба заказов написана. Ни одной
        # объявленной и ненаписанной службы в контракте не осталось, и потому
        # отказ проверяется подставным именем из того же реестра.
        assert not hasattr(client, "settings_service"), "выдуманная служба не должна быть видна"

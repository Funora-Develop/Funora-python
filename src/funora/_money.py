...
не будет: она требует вывести код валюты из символа.

УТОЧНЕНО 31.08.2026. Прежде здесь стояло «символ не наблюдался ни разу» - это
неверно: знак наблюдается на каждой странице с суммами, а в шапке стоит
переключатель, чьи пункты несут код ISO 4217 в data-cy.

Неверен был вывод, а не наблюдение. Переключатель говорит о НАСТРОЙКЕ ПОКАЗА, и
сумма заказа за ней не следует: после переключения на доллары список продаж
остался с рублёвым знаком у всех тридцати четырёх сумм. Сумма заказа показана в
валюте ЗАКАЗА.

Придуманная таблица символов молча приписала бы чужую валюту чужому заказу, и
заметил бы это не разработчик, а продавец. Записано в

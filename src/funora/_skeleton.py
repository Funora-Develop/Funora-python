...
Слово «не может» точно только для текста и значений атрибутов: там сохраняется
подпись, а не значение, и обратного преобразования не существует. С путями в
ссылках слабее, и об этом честнее сказать вслух. Путь сохраняется, потому что по
его форме узнаётся назначение ссылки, а обезличивание сегментов - правило, а не
доказательство: сегмент с цифрой или с нелатинскими знаками заменяется целиком,
но сегмент из одних латинских букв проходит как есть. Для FunPay это безопасно,
там пользователь адресуется числом, но правило держится на этом наблюдении, а не
на устройстве формата.

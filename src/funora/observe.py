...
def observe(
    *,
    ...
    # ЧУЖОЙ СНИМОК НЕ ЗАТИРАЕТСЯ. Имя строится по пути с обезличенными
    # сегментами, и потому у РАЗНЫХ страниц оно совпадает: /users/111/ и
    # /users/222/ обе дают users_n.
    #
    # Снимок невосполним. Он снят в конкретную минуту, при конкретном состоянии
    # аккаунта, и второй раз такой же не сделать: заказы закрываются, диалоги
    # уходят вниз, лоты правятся. Затирание молча уничтожало бы свидетельство,
    # на котором стоят правила извлечения.
    #
    # Совпадающее содержимое переписывается: повторная съёмка той же страницы -
    # обычное дело, и мешать ей незачем.
    target = out_dir / f"{stem}.skeleton.txt"
    if target.is_file() and target.read_text(encoding="utf-8") != skeleton:
        print(
            f"НЕ СОХРАНЕНО: под именем {target.name} уже лежит ДРУГОЙ снимок. "
            "Имя строится по пути, а идентификаторы в нём обезличены, поэтому у "
            "разных страниц имя совпадает. Снимок невосполним: он снят в "
            "конкретную минуту и вторым таким же не будет. "
            f"Снимайте в свой каталог: --out {out_dir}\\имя-снимка",
            file=sys.stderr,
        )
        return 1

    target.write_text(skeleton, encoding="utf-8", newline="\n")

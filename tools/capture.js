...
;(function () {
  ...
  const onUnload = () => {
    // ИМЯ ОБЯЗАНО БЫТЬ ДАНО ЗАРАНЕЕ, и без него запись теряется молча. Это и
    // случилось 31.08.2026: наблюдатель позвал funora.watch() без имени, нажал
    // «Сохранить», страница ушла - и записи не стало.
    //
    // Предупредить здесь уже поздно: консоль уходит вместе со страницей.
    // Поэтому предупреждает watch(), в тот момент, когда исправить ещё можно.
  ...
  window.funora = {
     ...
     */
    watch(name) {
      // Имя спрашивается ЗАРАНЕЕ, потому что действие может увести со
      // страницы. Уйдя, она уносит и консоль: позвать stop("имя") станет
      // некому, а маяк без имени не отправит ничего.
      if (!name) {
        console.warn(
          'funora: БЕЗ ИМЕНИ. Если действие уводит со страницы - сохранение, ' +
          'переход, отправка формы, - запись пропадёт: позвать stop() будет ' +
          'уже негде. Зовите funora.watch("имя") сразу.',
        )
      }
